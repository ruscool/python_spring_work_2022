...
def mes(message):
    ...
    if message.text == 'Тест':
        ...
        cycle(message, quest, keyboard_back, q)
...
# @bot.message_handler()
def cycle(message, quest, keyboard_back,id_question, f=None, check=0):
    check = logger.verify_check()
    check = check[0]
    print('verify', check)
    while check == 0:

        if f == 'callback_worker':
            arg = logger.for_answers()
            return arg
        else:
            l_num = []
            # button1 = cl_menu_keyboard.back_menu()
            # keyboard_back2 = types.ReplyKeyboardMarkup(resize_keyboard=True).add(*button1)
...
def choice_answer(message, quest, keyboard_back, id_question):
    bot.send_message(message.chat.id, f'Ответ принят 555')

    bot.register_next_step_handler_by_chat_id(message, cycle(message, quest, keyboard_back, id_question))
